...
describe("FiniteField Contract", function () {
  ...
  it("should add elements correctly", async function () {
    ...
    const sum: ZpStructOutput = await finiteField.sum(toZpStruct(elementA), toZpStruct(elementB));
    expect(sum.value).to.equal(1); // (3 + 5) mod 7 = 1
  });

  it("should subtract elements correctly", async function () {

    const diff: ZpStructOutput = await finiteField.sub(toZpStruct(elementA), toZpStruct(elementB));
    expect(diff.value).to.equal(5); // (3 - 5 + 7) mod 7 = 5
  });

  it("should multiply elements correctly", async function () {

    const prod: ZpStructOutput = await finiteField.mul(toZpStruct(elementA), toZpStruct(elementB));
    expect(prod.value).to.equal(1); // (3 * 5) mod 7 = 15 mod 7 = 1
  });
  it("should calculate the inverse correctly", async function () {
    const elementA: ZpStructOutput = await finiteField.createElement(3);
    const inv: ZpStructOutput = await finiteField.inverse(toZpStruct(elementA));
    expect(inv.value).to.equal(5);
  })

  it("should divide elements correctly", async function () {

    const div: ZpStructOutput = await finiteField.div(toZpStruct(elementA), toZpStruct(elementB));
  ...
  it("should revert on division by zero", async function () {
    const elementA: ZpStructOutput = await finiteField.createElement(3);
    const elementB: ZpStructOutput = await finiteField.createElement(0);
